...
gen_kubeconfig_base_config() {
  ...
  acp_host=$(yq --unwrapScalar '.acp.baseUrl' "$config_yaml")
  acp_token=$(yq --unwrapScalar '.acp.token' "$config_yaml")
  acp_test_cluster=$(yq --unwrapScalar '.acp.cluster' "$config_yaml")
